...
-- Create trigger for tracking delivery attempts if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 
    FROM pg_trigger 
    WHERE tgname = 'email_delivery_tracking'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 
    FROM pg_policies 
    WHERE policyname = 'Service role can manage templates'
    AND tablename = 'templates'
    AND schemaname = 'emails'
  ) THEN
